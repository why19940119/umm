/**
 * ============================================================================
 * US Market Sector Monitor Pipeline v1.13 — Clean Manual-Control Edition
 *
 ...
 * 新增：Cross-Asset Extension v1.0 prat1,2,3,4,5,6,7
 * ============================================================================
 */
...
function runDailyMarketPipeline() {
  ...
  try {
    ...
    Logger.log(`[Dashboard Publish] ${publishRes.status}, ${publishRes.fileName}`);
...
function manualGenerateAIReport() {
  const lock = LockService.getScriptLock();
  if (!lock.tryLock(30000)) throw new Error('已有另一個 Pipeline 或手動程序正在執行，請稍後再試。');
  try {
    ...
    if (result.status !== 'SUCCESS') throw new Error(`手動生成 AI 報告失敗：${result.status}`);
...
function manualSendAIReport() {
  ...
  try {
    ...
    if (result.status !== 'SUCCESS') throw new Error(`手動寄送 Email 失敗：${result.status}`);
...
function createDailySnapshot() {
  ...
  if (!sectorsSheet || !snapshotSheet) throw new Error(`找不到必要工作表：${CONFIG.SHEET_SECTORS} 或 ${CONFIG.SHEET_SNAPSHOT}`);

  const marketDateET = getUSMarketDateET_();
  if (!isAfterUsMarketClose_()) return { snapshotId: '', marketDateET: marketDateET, status: 'MARKET_NOT_CLOSED' };

  const etDay = Utilities.formatDate(new Date(), 'America/New_York', 'u');
  if (etDay === '6' || etDay === '7' || isUsMarketHoliday_(marketDateET)) return { snapshotId: '', marketDateET: marketDateET, status: 'NON_TRADING_DAY' };

  const existingSnapshotId = checkExistingSnapshot_(snapshotSheet, marketDateET);
  if (existingSnapshotId) return { snapshotId: existingSnapshotId, marketDateET: marketDateET, status: 'SKIPPED_ALREADY_EXISTS' };

  const lastRow = sectorsSheet.getLastRow();
  if (lastRow < 2) return { snapshotId: '', marketDateET: marketDateET, status: 'FAILED_INCOMPLETE' };
  ...
  for (const row of rawData) {
    ...
    if (numericValues.some(isInvalidNumericValue_)) return { snapshotId: '', marketDateET: marketDateET, status: 'FAILED_INCOMPLETE' };
    const category = String(row[11]).trim();
    if (!category || category.includes('#') || category.includes('載入中') || category.includes('Loading')) return { snapshotId: '', marketDateET: marketDateET, status: 'FAILED_INCOMPLETE' };
    validRows.push(row);
  }

  const tickers = validRows.map(row => String(row[1]).trim().toUpperCase());
  const uniqueTickers = new Set(tickers);
  const complete = validRows.length === CONFIG.EXPECTED_ETF_COUNT && uniqueTickers.size === CONFIG.EXPECTED_ETF_COUNT && [...CONFIG.ALLOWED_ETFS].every(ticker => uniqueTickers.has(ticker));
  if (!complete) return { snapshotId: '', marketDateET: marketDateET, status: 'FAILED_INCOMPLETE' };
  ...
  const rowsToInsert = validRows.map(row => [snapshotId, nowHKT, marketDateET, 'COMPLETE', row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[9], row[10], row[11]]);
...
function generateAIReport_(snapshotId, marketDateET, mode) {
  ...
  if (!snapshotSheet || !aiReportSheet) throw new Error(`找不到必要工作表：${CONFIG.SHEET_SNAPSHOT} 或 ${CONFIG.SHEET_AI_REPORT}`);
  if (!isManual && checkExistingAIReport_(aiReportSheet, snapshotId)) return { status: 'SKIPPED_ALREADY_EXISTS' };

  const apiKey = PropertiesService.getScriptProperties().getProperty(CONFIG.PROP_OPENROUTER_KEY);
  if (!apiKey) throw new Error('找不到 OPENROUTER_API_KEY，請檢查 Script Properties。');
  ...
  if (targetRows.length !== CONFIG.EXPECTED_ETF_COUNT) return { status: 'FAILED_INCOMPLETE_SNAPSHOT' };

  const dataJson = JSON.stringify(targetRows.map(row => ({ sector: row[4], ticker: row[5], lastPrice: row[7], change5d: row[11], change20d: row[12], range52w: row[10], classification: row[13] })));
  ...
  const payload = { model: CONFIG.AI_MODEL, messages: [{ role: 'user', content: prompt }], temperature: 0.1, max_tokens: 1500, reasoning: { effort: 'none', exclude: true } };
  const options = { method: 'post', contentType: 'application/json', headers: { Authorization: `Bearer ${apiKey}`, 'HTTP-Referer': 'https://us-market-monitor', 'X-Title': 'US Market Monitor' }, payload: JSON.stringify(payload), muteHttpExceptions: true };
  ...
  try {
    ...
    const cleanReport = sanitizeAiOutput_(result.choices?.[0]?.message?.content);

    aiReportSheet.appendRow([new Date(), marketDateET, targetRows[0][1], CONFIG.AI_MODEL, isManual ? 'MANUAL_SUCCESS' : 'SUCCESS', snapshotId, cleanReport, '', isManual ? 'MANUAL_PENDING' : 'PENDING']);
    SpreadsheetApp.flush();
    return { status: 'SUCCESS' };
  } catch (error) {
    aiReportSheet.appendRow([new Date(), marketDateET, targetRows.length ? targetRows[0][1] : '', CONFIG.AI_MODEL, isManual ? 'MANUAL_API_ERROR' : 'API_ERROR', snapshotId, '', error.toString(), 'NOT_SENT']);
    SpreadsheetApp.flush();
...
function sendLatestManualReport_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const aiReportSheet = ss.getSheetByName(CONFIG.SHEET_AI_REPORT);
  if (!aiReportSheet) throw new Error(`找不到必要工作表：${CONFIG.SHEET_AI_REPORT}`);
...
function sendReportRow_(report, isManual) {
  if (!CONFIG.RECIPIENT_EMAILS || !CONFIG.RECIPIENT_EMAILS.length) return { status: 'FAILED_NO_RECIPIENTS' };
  ...
  try {
    ...
    const subject = isManual ? `🧪 [手動報告] 美股板塊 AI 分析報告 (${marketDateET})` : `📈 美股板塊 AI 分析報告 (${marketDateET})`;
...
function sanitizeAiOutput_(content) {
  ...
  if (startIndex === -1) throw new Error('AI 回應找不到正式報告起點【市場結構】，已拒絕寫入及寄送。');
  const finalReport = rawText.slice(startIndex).trim();
  const requiredSections = ['【市場結構】', '【最強板塊】', '【最弱板塊】', '【短期轉變】', '【明日監察重點】', '【資料限制】'];
  const missing = requiredSections.filter(section => !finalReport.includes(section));
  if (missing.length) throw new Error(`AI 正式報告格式不完整，缺少：${missing.join('、')}。`);
  const forbidden = [/thinking process/i, /<think>/i, /<\/think>/i, /reasoning process/i, /^analysis\s*:/im];
  if (forbidden.some(pattern => pattern.test(finalReport))) throw new Error('AI 正式報告內容仍包含內部推理文字，已拒絕寫入及寄送。');
  return finalReport;
}
...
function getLatestCompleteSnapshot_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const snapshotSheet = ss.getSheetByName(CONFIG.SHEET_SNAPSHOT);
  if (!snapshotSheet) throw new Error(`找不到必要工作表：${CONFIG.SHEET_SNAPSHOT}`);
  ...
  for (let i = data.length - 1; i >= 1; i--) {
    ...
    const isComplete = targetRows.length === CONFIG.EXPECTED_ETF_COUNT && tickers.size === CONFIG.EXPECTED_ETF_COUNT && [...CONFIG.ALLOWED_ETFS].every(ticker => tickers.has(ticker));
    if (isComplete) return { snapshotId: snapshotId, marketDateET: normalizeDate_(row[2]) };
  }
  throw new Error('找不到完整的 11 ETF Daily_Snapshot。');
}

function getUSMarketDateET_() { return Utilities.formatDate(new Date(), 'America/New_York', 'yyyy-MM-dd'); }
function isAfterUsMarketClose_() { return Number(Utilities.formatDate(new Date(), 'America/New_York', 'HH')) >= 18; }
function isUsMarketHoliday_(marketDateET) {
  ...
  return rows.some(([date, isTradingDay]) => date.trim() === marketDateET && isTradingDay.trim().toUpperCase() === 'FALSE');
}
function isInvalidNumericValue_(value) {
  const text = String(value).trim();
  return value === '' || value === null || text.includes('#') || text.includes('載入') || text.includes('Loading') || !Number.isFinite(Number(value));
}
function normalizeDate_(value) { return value instanceof Date ? Utilities.formatDate(value, 'America/New_York', 'yyyy-MM-dd') : String(value).trim(); }
function checkExistingSnapshot_(sheet, marketDateET) {
  ...
  for (const [snapshotId, rows] of Object.entries(grouped)) {
    ...
    const complete = rows.length === CONFIG.EXPECTED_ETF_COUNT && tickers.size === CONFIG.EXPECTED_ETF_COUNT && [...CONFIG.ALLOWED_ETFS].every(ticker => tickers.has(ticker));
...
function checkExistingAIReport_(sheet, snapshotId) {
  const data = sheet.getDataRange().getValues();
  for (let i = 1; i < data.length; i++) {
    if (data[i][5] === snapshotId && data[i][4] === 'SUCCESS') return true;
  }
  return false;
}
function setupMarketCalendar() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let calendarSheet = ss.getSheetByName(CONFIG.SHEET_CALENDAR) || ss.insertSheet(CONFIG.SHEET_CALENDAR);
  calendarSheet.clear();
  const calendarData = [['Date', 'Is_Trading_Day'],['2026-01-01', 'FALSE'],['2026-01-19', 'FALSE'],['2026-02-16', 'FALSE'],['2026-04-03', 'FALSE'],['2026-05-25', 'FALSE'],['2026-06-19', 'FALSE'],['2026-07-03', 'FALSE'],['2026-09-07', 'FALSE'],['2026-11-26', 'FALSE'],['2026-12-25', 'FALSE']];
